...
- `assets/examples/*.png`：v0.2.0 风格校准示例资产，用来判断留白、角色参与方式、隐喻浓度和中文批注密度；只能校准方向，不能照抄构图。

v0.2.0 恢复收录 `assets/examples/` 作为新版校准资产，但它们不是模板库。需要风格判断时，优先综合 `references/style-dna.md`、`references/xiaofan-ip.md`、`references/composition-patterns.md` 与 v4 动作参考表，不要把任何单张 examples 当成默认底稿。
